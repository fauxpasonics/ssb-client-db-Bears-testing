...
CREATE VIEW [ods].[vw_LoadDimCustomer_Infogroup] AS ( --Change "SOURCESYSTEM" to the correct source
...
--base set
		SELECT
			'Bears' AS [SourceDB] --Client Source Database such as Raiders or Raiders_Reporting
			, 'Infogroup' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(z.Sequence_Number AS NVARCHAR(200)) [SSID] -- Required, must be cast as NVARCHAR(200)
			...
			, CAST(z.BE_Company_Name AS NVARCHAR (50)) AS [CompanyName] 
			...
			/*AddressPrimary*/
            , CASE WHEN ISNULL(z.Address_Line2,'')='' THEN z.Address_Line1 ELSE z.Address_Line1+' '+z.Address_Line2 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street eg. CASE WHEN ISNULL(address2,'') <> '' THEN z.street + ' ' + z.address2 ELSE z.street END AS [AddressPrimaryStreet]
			...
			, z.AH1_LOCAL_ADDRESS AS [AddressOneStreet]
			, NULL AS [AddressOneSuite] -- ALWAYS LEAVE THIS NULL!!!!!!!!!!!!!!!!
			, z.AH1_LONG_CITY_NAME AS [AddressOneCity] 
			, z.AH1_STATE_ABBREVIATION AS [AddressOneState] 
			, z.AH1_ZIP_CODE AS [AddressOneZip] 
			...
			/*Phone*/
			, LEFT(z.PhoneDay,25) AS [PhonePrimary] -- For Coalescing values make sute they are wrapped in a NULLIF for values that = '' eg COALESCE(NULLIF(z.mobile,''),z.phone) AS [PhonePrimary]
			, LEFT(z.PhoneEve,25) AS [PhoneHome]
			, LEFT(NULL,25) AS [PhoneCell]
			, LEFT(z.BE_Telephone_NumberF1,25) AS [PhoneBusiness]
			, LEFT(z.BE_Fax_NumberF1,25) AS [PhoneFax]
			...
			, z.BE_Production_Date_Formatted [SSCreatedDate] --createddate
		...
		--SELECT COUNT(*)			
		FROM ods.Infogroup_Appends z WITH (NOLOCK) --Change To Source Table
		WHERE 1=1
