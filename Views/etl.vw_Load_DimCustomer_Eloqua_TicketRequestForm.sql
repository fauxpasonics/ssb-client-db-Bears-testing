...
CREATE VIEW [etl].[vw_Load_DimCustomer_Eloqua_TicketRequestForm] AS 
...
--base set
		SELECT
			...
			, 'Eloqua_TicketRequest' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			...
			/*Extended Attributes*/
			, CAST(z.ContactId AS NVARCHAR(100)) AS[ExtAttribute1] --nvarchar(100)
			...
			, 0 AS IsBusiness
			, 0 AS isDeleted
			, CASE WHEN ISNULL(CAST(z.ContactId AS VARCHAR(100)),'') != '' THEN 'ELO-'+CAST(z.ContactId AS VARCHAR(100)) ELSE NULL END AS [Customer_Matchkey] /*Standard for macthkeys are ‘TM-xxxx’, ‘Pac-xxxx’ depending on source, 
											Customer Matchkey needs to have fairly complicated logic. 
											See Writup for details.*/
		--SELECT COUNT(*)			
		FROM src.vw_Eloqua_TicketRequestForm_Contacts z WITH (NOLOCK) 
		WHERE 1=1
