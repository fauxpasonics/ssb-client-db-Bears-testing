...
            , CASE WHEN z.new_SSBCRMSYSTEMSSIDWinner IS NULL OR z.new_SSBCRMSystemSSIDWinnerSourceSystem IS NULL THEN NULL
			   ...
			   WHEN z.new_SSBCRMSystemSSIDWinnerSourceSystem = 'TM' THEN 'TM-' + new_SSBCRMSYSTEMSSIDWinner
...
        --SELECT COUNT(*)           
        FROM Bears_reporting.prodcopy.contact z WITH (NOLOCK) 
		LEFT JOIN dimcustomer dc ON CAST(z.contactid AS NVARCHAR(100)) = dc.ssid 
        WHERE 1=1
        AND DATEDIFF(DAY,z.modifiedon,GETDATE())<=10 OR DATEDIFF(DAY,z.copyloaddate,GETDATE())<=10 --This needs to be uncommented out after the first load, update the date fields
    ) a
 
)
