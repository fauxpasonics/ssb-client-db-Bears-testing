...
--base set
		SELECT 
			...
			, CAST(CASE WHEN ISNULL(company_name,'')<>'' THEN company_name WHEN ISNULL(name_first,'')='' AND ISNULL(name_last,'')<>'' AND ISNULL(name_last,'')<>'LASTNAME' THEN name_last
				ELSE company_name END AS NVARCHAR (50)) AS [CompanyName] 
			...
			, CASE WHEN ISNULL(name_first,'')='' AND ISNULL(name_last,'')<>''  THEN '' ELSE LEFT(name_last,100) END AS [LastName]
			, NULL AS [Suffix]
			, NULL AS [Fullname]
			...
			, 'TM-' + CAST(acct_id AS NVARCHAR(100)) + ':' + CAST(cust_name_id AS NVARCHAR(100)) AS[ExtAttribute6] 
			...
			, GETDATE() [CreatedDate]
			, 0 AS IsDeleted
			, CAST(CASE WHEN name_type = 'C' THEN 1  WHEN ISNULL(company_name,'')<>'' THEN 1 WHEN ISNULL(name_first,'')='' AND ISNULL(name_last,'')<>'' AND ISNULL(name_last,'')<>'LASTNAME' THEN 1 ELSE 0 END AS BIT) IsBusiness
			, CASE WHEN CAST(primary_code AS NVARCHAR (50)) = 'Primary' THEN CONCAT('TM-',acct_id)
				ELSE NULL
