...
CREATE VIEW [etl].[vw_Load_DimCustomer_Eloqua_ContactUsForm] AS 
...
--base set
		SELECT
			...
			, 'Eloqua_ContactUs' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(z.SSID AS NVARCHAR(50)) [SSID] -- Required
			...
			, CONVERT(VARCHAR(30),z.birthdate, 102) AS [Birthday] -- field is sensitive see writup in sharepoint for an example of how complicated it can get
			, NULL AS [Gender] 
			, 0 [MergedRecordFlag]
			, NULL [MergedIntoSSID]
			...
			/*AddressPrimary*/
            , z.address1 AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
			, z.city AS [AddressPrimaryCity] 
			, z.stateOrProvince	AS [AddressPrimaryState] 
			, z.zipOrPostalCode AS [AddressPrimaryZip] 
			...
			/*Extended Attributes*/
			, z.ContactId AS[ExtAttribute1] --nvarchar(100)
			...
			, 0 AS IsBusiness
			, 0 AS isDeleted
			, CASE WHEN ISNULL(z.ContactId,'') != '' THEN 'ELO-' + CAST(z.ContactId AS VARCHAR(100)) ELSE NULL END AS [Customer_Matchkey] /*Standard for macthkeys are ‘TM-xxxx’, ‘Pac-xxxx’ depending on source, 
											Customer Matchkey needs to have fairly complicated logic. 
											See Writup for details.*/
		--SELECT COUNT(*)			
		FROM src.vw_Eloqua_ContactUsForm_Contacts z WITH (NOLOCK) 
		WHERE 1=1
		AND DATEDIFF(DAY,z.CreatedAt,GETDATE())<=10 --This needs to be uncommented out after the first load, update the date fields
		--OR z.lastname = 'twarling'
	) a
