...
        --SELECT COUNT(*)           
        FROM prodcopy.vw_account z WITH (NOLOCK) 
        WHERE 1=1
        AND DATEDIFF(DAY,z.modifiedon,GETDATE())<=10 OR DATEDIFF(DAY,z.createdon,GETDATE())<=10 OR DATEDIFF(DAY,z.copyloaddate,GETDATE())<=5 --This needs to be uncommented out after the first load, update the date fields
    ) a
 
)
 
