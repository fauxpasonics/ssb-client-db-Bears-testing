...
SELECT * FROM 
	( SELECT    ssbid.SSB_CRMSYSTEM_CONTACT_ID
      ...
      FROM      dbo.FactTicketSales fts WITH ( NOLOCK )
                INNER JOIN dbo.DimPriceCode DimPriceCode WITH ( NOLOCK ) ON DimPriceCode.DimPriceCodeId = fts.DimPriceCodeId
                INNER JOIN dbo.DimTicketType DimTicketType WITH ( NOLOCK ) ON DimTicketType.DimTicketTypeId = fts.DimTicketTypeId
                INNER JOIN dbo.DimPlanType DimPlanType WITH ( NOLOCK ) ON DimPlanType.DimPlanTypeId = fts.DimPlanTypeId
                INNER JOIN dbo.DimSeatType DimSeatType WITH ( NOLOCK ) ON DimSeatType.DimSeatTypeId = fts.DimSeatTypeId
                ...
                INNER JOIN dbo.DimDate DimDate WITH ( NOLOCK ) ON DimDate.DimDateId = fts.DimDateId
                INNER JOIN dbo.DimSeason DimSeason WITH ( NOLOCK ) ON DimSeason.DimSeasonId = fts.DimSeasonId
                INNER JOIN dbo.DimEvent DimEvent WITH ( NOLOCK ) ON DimEvent.DimEventId = fts.DimEventId
                INNER JOIN dbo.dimcustomerssbid ssbid WITH ( NOLOCK ) ON ssbid.DimCustomerId = fts.DimCustomerId
                INNER JOIN dbo.DimClassTM DimClassTM WITH ( NOLOCK ) ON DimClassTM.DimClassTMId = fts.DimClassTMId
                INNER JOIN dbo.DimSalesCode DimSaleCode WITH ( NOLOCK ) ON DimSaleCode.DimSalesCodeId = fts.DimSalesCodeId
                INNER JOIN dbo.DimPromo DimPromo WITH ( NOLOCK ) ON DimPromo.DimPromoID = fts.DimPromoId
                INNER JOIN dbo.DimSeat DimSeat WITH ( NOLOCK ) ON DimSeat.DimSeatId = fts.DimSeatIdStart
                INNER JOIN dbo.DimItem DimItem WITH ( NOLOCK ) ON DimItem.DimItemId = fts.DimItemId
				...
				INNER JOIN dbo.DimTeam DimTeam  WITH ( NOLOCK ) ON DimEventHeader.OpponentDimTeamId = DimTeam.DimTeamId
				INNER JOIN dbo.DimPlan DimPlan WITH ( NOLOCK ) ON DimPlan.DimPlanId = fts.DimPlanId
    ) cy
	UNION ALL 
	SELECT * FROM
	( SELECT    ssbid.SSB_CRMSYSTEM_CONTACT_ID
