...
CREATE PROCEDURE [etl].[SSB_ProcessStandardMerge_ToOds_old20171127]
(
	@Target VARCHAR(256),
	@Source VARCHAR(256),
...
--		only delete records that are not already soft-deleted.  Otherwise, they will never get set to active again in the merge below
DELETE mySource
FROM #SrcData mySource
	JOIN ' + @Target + ' myTarget
	ON ' + @JoinString + ' and mySource.ETL_DeltaHashKey = myTarget.ETL_DeltaHashKey and myTarget.ETL_DeltaHashKey = 0;
